...
def load_train_bbox(num_classes, label_dict, bbox_dir):
    ...
    return final_dict
def load_val_bbox(all_imgs, bb_dict):
    final_bbox_dict = {}
    for i in range(len(all_imgs)):
        img_name = all_imgs[i][0].split('/')[-2] + '/' + all_imgs[i][0].split('/')[-1]
        final_bbox_dict[all_imgs[i][0]] = bb_dict[img_name]
...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root, ddt_path, input_size=256, crop_size=224,train=True, transform=None, target_transform=None, loader=default_loader):
        ...
        self.num_classes = len(self.img_dataset.classes)
        ...
        bb_dict = get_bbox_dict(root)
        if self.train:
            #load train bbox
            self.bbox_dict = load_train_bbox(self.num_classes, self.label_class_dict,self.ddt_path)
        else:
            #load test bbox
            self.bbox_dict = load_val_bbox(self.img_dataset.imgs, bb_dict)
